...
local ModelPath = "models/mad_centipedemob.mdl"
...
hook.Add("PostDrawOpaqueRenderables", "DrawSoloLevelingNPCHealthBarCentipede", function()
    for _, ent in ipairs(ents.FindByClass("mad_centipede_mob")) do
        if IsValid(ent) then
            DrawHealthBar(ent)
        end
    end
