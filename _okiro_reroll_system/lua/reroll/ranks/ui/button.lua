...
function PANEL:Paint(w, h)
    ...
    elseif not MagieReroll.Ranks.CanAfford(LocalPlayer()) and not self.animationFinished then
    ...
    MagieReroll.UI.Text( sl_data3["rang"] or 0, 'mr.font1', w*.5, h*.5, self.currentColors.rect, 1, 1)

    -- MagieReroll.UI.Image(w - imageSize - scale(20), h * .5 - imageSize * .5, imageSize, imageSize, mats.coin, colors.white)
    -- MagieReroll.UI.Text(MagieReroll.Ranks.Config.Price.. 'M ₩', 'mr.font1', w - scale(60), h*.5, self.currentColors.rect, 2, 1)
...
function PANEL:DoClick()
    ...
    if self.animationFinished then
        ...
        net_Start('MagieRanks_DoReroll')
        net_SendToServer()
        return
    end
    
    if not MagieReroll.Ranks.CanAfford(LocalPlayer()) then
...
vgui_Register('MagieRanks_Button', PANEL, 'DButton')