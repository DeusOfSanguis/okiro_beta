...
local ModelPath = "models/mad_werewolfmob.mdl"
...
function ENT:Initialize()
    ...
    if IsValid(self.ClientModel) then
        ...
        self.ClientModel:SetSkin(math.random(0, 2))
...
hook.Add("PostDrawOpaqueRenderables", "DrawSoloLevelingNPCHealthBarWerewolf", function()
    for _, ent in ipairs(ents.FindByClass("mad_werewolf_mob")) do
        if IsValid(ent) then
            DrawHealthBar(ent)
        end
    end
