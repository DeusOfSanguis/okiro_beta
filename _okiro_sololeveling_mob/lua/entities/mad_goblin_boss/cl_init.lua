...
local ModelPath = "models/mad_goblinmage.mdl"
...
-- Vérifie et met à jour l'animation basée sur l'état
function ENT:CheckAndPlayAnimation()
    ...
    local state = "idle" -- Animation idle par défaut

    if self.Attacking then
        state = GetRandomAttackAnimation() -- Animation d'attaque aléatoire
    elseif self:GetNWBool("IsMoving") then
        state = "run"

        if self.CurrentTarget and IsValid(self.CurrentTarget) then
            local attackRange = self:GetNWInt("AttackRange", 100) + 50

            if self.NextAttack and self.NextAttack > CurTime() then
                if self:GetPos():Distance(self.CurrentTarget:GetPos()) <= attackRange then
                    state = GetRandomAttackAnimation() -- Animation d'attaque aléatoire
                    self.NextAttack = CurTime() + 8 -- Attaque toutes les secondes
                else
                    state = "run"
                end
            end
    ...
    elseif self.CurrentTarget and IsValid(self.CurrentTarget) then
        local attackRange = self:GetNWInt("AttackRange", 100) + 50
    ...
    if state ~= self.LastState and not self.onCustomSequence then
        ...
        -- Vérifie si l'état est une animation d'attaque
        local isAttackAnimation = false
        for _, anim in ipairs(AttackAnimations) do
            if anim.Sequence == state then
        ...
        if isAttackAnimation then
            ...
            self:PlaySequenceOnce(state, function()
                -- print("Animation d'attaque terminée.")
            end)
            timer.Simple(1, function()
                if not IsValid(self) then return end
                self.Attacking = false
                self:PlayLoopingAnimation("run")
                self.LastState = "run"
            end)
        else
            self:PlayLoopingAnimation("run")
...
local function DrawHealthBar(ent)
    ...
    local boneId = ent.ClientModel:LookupBone("mixamorig:Head")
...
hook.Add("PostDrawOpaqueRenderables", "DrawSoloLevelingNPCHealthBarGoblinBoss", function()
    for _, ent in ipairs(ents.FindByClass("mad_goblin_boss")) do
        if IsValid(ent) then
            DrawHealthBar(ent)
        end
    end
