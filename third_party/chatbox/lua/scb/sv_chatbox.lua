...
local chatRestrictionEnabled = true

hook.Add("PlayerSay", "SCB_BlockChatForUsers", function(ply, text, teamChat)
    if not chatRestrictionEnabled then return text end
    
    if not ply:IsSuperAdmin() then 
        if string.sub(text, 1, 1) == "/" then
            local allowedCommands = {"/advert", "/me", "/give", "/dropmoney"}
            for _, cmd in ipairs(allowedCommands) do
                if string.sub(text, 1, #cmd) == cmd then
                    return text 
                end
            end
        end
        
        ply:ChatPrint("Le chat est désactivé. Vous pouvez uniquement utiliser certaines commandes.")
        return ""
    end
...
concommand.Add("toggle_chatblock", function(ply)
    if IsValid(ply) and not ply:IsSuperAdmin() then return end 

    chatRestrictionEnabled = not chatRestrictionEnabled 
    local state = chatRestrictionEnabled and "activé" or "désactivé"
    local message = "Le chat a été " .. state .. " par " .. (IsValid(ply) and ply:Nick() or "Console")
...
-- 		end
-- 		if v.id_number == "1401732485" then continue end
