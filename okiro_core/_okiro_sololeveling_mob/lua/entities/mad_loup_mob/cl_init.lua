...
local ModelPath = "models/mad_loup.mdl"
...
hook.Add("PostDrawOpaqueRenderables", "DrawSoloLevelingNPCHealthBarLoup", function()
    for _, ent in ipairs(ents.FindByClass("mad_loup_mob")) do
        if IsValid(ent) then
            DrawHealthBar(ent)
        end
    end
