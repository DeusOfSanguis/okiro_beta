...
local ModelPath = "models/mad_loupevo2.mdl"
...
local function DrawHealthBar(ent)
    ...
    local boneId = ent.ClientModel:LookupBone("G_head_def")
...
hook.Add("PostDrawOpaqueRenderables", "DrawSoloLevelingNPCHealthBarLoupBoss", function()
    for _, ent in ipairs(ents.FindByClass("mad_loup_boss")) do
        if IsValid(ent) then
            DrawHealthBar(ent)
        end
    end
