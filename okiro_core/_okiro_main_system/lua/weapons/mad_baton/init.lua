...
function SWEP:DamageCombo()
    ...
    for i = -angleOffset, angleOffset, 5 do
        ...
        local slash = {
            ...
            mins = Vector(-radius/2, -radius/2, 0),
            maxs = Vector(radius/2, radius/2, 0)
        }
        ...
        if tr.Hit and IsValid(tr.Entity) then
            ...
            if (tr.Entity:IsPlayer() or isNPC or tr.Entity:IsNextBot()) and not hitEntities[tr.Entity] then
                ...
                tr.Entity:TakeDamage((ply.damage + self.BonusDegats)/4, ply, self)
                ...
                local function ApplyParticles()
                    ...
                    local particlePos = torsoAttachID == 0 and (tr.Entity:GetPos() + tr.Entity:GetUp() * 40) or nil
                    
                    if ply:GetNWInt("Combo") == 2 or ply:GetNWInt("Combo") == 3 then
                        if torsoAttachID == 0 then
                            ParticleEffect("mudrock", particlePos, Angle(0, 0, 0), tr.Entity)
                            ParticleEffect("slashhit_base", particlePos, Angle(0, 0, 0), tr.Entity)
                        else
                    ...
                    if torsoAttachID == 0 then
                        ParticleEffect("slashhit_helper_3", particlePos, Angle(0, 0, 0), tr.Entity)
                    else
	...
	if tr.Hit then
		if IsValid(tr.Entity) then
			...
			if (tr.Entity:IsPlayer() or isNPC or tr.Entity:IsNextBot()) then
				tr.Entity:TakeDamage(dmg+self.BonusDegats, ply, self)
				tr.Entity:EmitSound(hitjoueur[math.random(1, 3)], 75, math.random(70, 130), 0.8, CHAN_AUTO)
				util.ScreenShake(pos, 3, 50, 0.5, 150)
				local torsoAttachID = tr.Entity:LookupAttachment("chest")
				if torsoAttachID == 0 then
...
function SWEP:DamageLaunch(dmg, isburn, burntime, isfreeze, freezetime, havefreezeefffect, freezeeffect, havehitsound, hitsound, minsound, maxsound, isrepeat, reptime, particletable, speed)
	local ply = self:GetOwner()

	self.shot = ents.Create("ent_config_launch")
	--------------------------------------------------------------------------
	self.shot.Damage = dmg+self.BonusDegats/2
	self.shot.Burn = isburn
	self.shot.BurnTime = burntime
	--------------------------------------------------------------------------
	self.shot.Freeze = isfreeze
	self.shot.FreezeTimer = freezetime
	self.shot.HaveFreezeEffect = havefreezeefffect
	self.shot.FreezeEffect = freezeeffect
	--------------------------------------------------------------------------
	self.shot.HaveHitSound = havehitsound
	self.shot.HitSound = hitsound
	self.shot.MinSound = minsound
	self.shot.MaxSound = maxsound
	self.shot.HaveRepeat = isrepeat
	self.shot.RepeatTime = reptime
	--------------------------------------------------------------------------
	self.shot:SetPos(ply:GetPos() + Vector(0,0,50) + ply:GetForward()*75)
	self.shot:SetOwner(ply)
	self.shot:SetAngles(ply:EyeAngles())
	self.shot:Spawn()
	self.shot:GetPhysicsObject():EnableMotion(true)
	self.shot:SetCollisionGroup(COLLISION_GROUP_IN_VEHICLE)
	...
	timer_Simple(0.0001, function()
		for _, particle in ipairs(particletable) do
			ParticleEffectAttach(particle, 4, self.shot, 0)
		end
	end)

	local phys = self.shot:GetPhysicsObject()
	phys:EnableGravity(false)
	phys:SetVelocity( self.shot:GetForward() * speed )

end

function SWEP:DamageLaunch2(dmg, isburn, burntime, isfreeze, freezetime, havefreezeefffect, freezeeffect, havehitsound, hitsound, minsound, maxsound, isrepeat, reptime, particletable, speed, model, scale, material, zone_dist)
	local ply = self:GetOwner()
	...
	self.shot.RepeatTime = reptime
	--------------------------------------------------------------------------
	self.shot:SetPos(ply:GetPos() + ply:GetForward()*75 + ply:GetUp()*50)
	self.shot:SetOwner(ply)
	self.shot:SetAngles(ply:EyeAngles())
	self.shot:Spawn()
	self.shot:GetPhysicsObject():EnableMotion(true)
	...
	self.appa = ents.Create("prop_physics")
    
    self.appa:SetModel(model)
	self.appa:SetPos(self.shot:GetPos())
	self.appa:SetAngles(ply:EyeAngles())
	self.appa:SetParent(self.shot) 
    self.appa:Spawn()
	self.appa:SetCollisionGroup(COLLISION_GROUP_IN_VEHICLE)
	self.appa:SetModelScale(scale)
	self.appa:SetMaterial(material)
    
    -- Désactiver la physique
    self.appa:SetMoveType(MOVETYPE_NONE)
    self.appa:SetSolid(SOLID_NONE)

	self.shot.ZoneDist = zone_dist

	timer_Simple(0.0001, function()
		for _, particle in ipairs(particletable) do
			ParticleEffectAttach(particle, 4, self.appa, 0)
		end
	end)
