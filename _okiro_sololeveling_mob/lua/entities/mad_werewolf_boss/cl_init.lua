...
local ModelPath = "models/mad_werewolfboss.mdl"
...
function ENT:CheckAndPlayAnimation()
    ...
    if state ~= self.LastState and not self.onCustomSequence then
        ...
        if state == "skill1" or state == GetRandomAttackAnimation() then
...
hook.Add("PostDrawOpaqueRenderables", "DrawSoloLevelingNPCHealthBarWerewolfBoss", function()
    for _, ent in ipairs(ents.FindByClass("mad_werewolf_boss")) do
        if IsValid(ent) then
            DrawHealthBar(ent)
        end
    end
