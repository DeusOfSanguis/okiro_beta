...
local ModelPath = "models/mad_igrisboss.mdl"
...
function ENT:Initialize()
    ...
    if IsValid(self.ClientModel) then
        ...
        self.ClientModel:SetModelScale(1.2)
...
function ENT:CheckAndPlayAnimation()
    ...
    if state ~= self.LastState and not self.onCustomSequence then
        ...
        if state == "Skill" or state == GetRandomAttackAnimation() then
...
function ENT:Think()
    ...
    if IsValid(self.ClientModel) then
        self.ClientModel:SetPos(self:GetPos() + Vector(0,0,-45))
...
local function DrawHealthBar(ent)
    ...
    local boneId = ent.ClientModel:LookupBone("Bip001 Head")
...
hook.Add("PostDrawOpaqueRenderables", "DrawSoloLevelingNPCHealthBarIgrisBoss", function()
    for _, ent in ipairs(ents.FindByClass("mad_igris_boss")) do
        if IsValid(ent) then
            DrawHealthBar(ent)
        end
    end
