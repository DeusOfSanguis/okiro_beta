...
local ModelPath = "models/mad_undeadboss.mdl"
...
function ENT:CheckAndPlayAnimation()
    ...
    if state ~= self.LastState and not self.onCustomSequence then
        ...
        if state == "attack2" or state == GetRandomAttackAnimation() then
...
hook.Add("PostDrawOpaqueRenderables", "DrawSoloLevelingNPCHealthBarUndeadBoss", function()
    for _, ent in ipairs(ents.FindByClass("mad_undead_boss")) do
        if IsValid(ent) then
            DrawHealthBar(ent)
        end
    end
