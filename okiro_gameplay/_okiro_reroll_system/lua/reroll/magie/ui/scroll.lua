...
function PANEL:Init()
    ...
    self.indicator:SetSize(scale(4), scale(126))
...
function PANEL:CreateItemPanels()
    ...
    for i = 1, VISIBLE_ITEM_COUNT + 4 do
        local panel = vgui_Create('MagieReroll_ItemPanel', self)
        panel:SetSize(FRAME_WIDTH, scale(126))
...
function PANEL:LoadPreviewItems()
    self.items = MagieReroll.UI.GetPreviewItems()
...
function PANEL:FinishAnimation(frame)
    ...
    if IsValid(frame) then
        ...
        timer_Create('MagieReroll_ShakeEffect', 0.01, shakeDuration * 100, function()
            if not IsValid(frame) then timer_Remove('MagieReroll_ShakeEffect') return end
            
            local shakeProgress = (CurTime() - shakeStart) / shakeDuration
            if shakeProgress >= 1 then
                frame:SetPos(origX, origY)
                timer_Remove('MagieReroll_ShakeEffect')
                return
            end
...
vgui_Register('MagieReroll_ScrollContainer', PANEL, 'Panel')