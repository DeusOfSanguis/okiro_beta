...
local ModelPath = "models/mad_oursboss.mdl"
...
function ENT:CheckAndPlayAnimation()
    ...
    if state ~= self.LastState and not self.onCustomSequence then
        ...
        if state == "Skill01" or state == GetRandomAttackAnimation() then
...
hook.Add("PostDrawOpaqueRenderables", "DrawSoloLevelingNPCHealthBarOursBoss", function()
    for _, ent in ipairs(ents.FindByClass("mad_ours_boss")) do
        if IsValid(ent) then
            DrawHealthBar(ent)
        end
    end
