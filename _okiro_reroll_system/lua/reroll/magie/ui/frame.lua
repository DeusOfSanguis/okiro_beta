...
function PANEL:Init()
    ...
    -- self.rerolls = sl_data3["magie"] or 0
    ...
    self.back.DoClick = function()
        MagieReroll.Config.BackButtonFunction()

        self:SafetyRemove()
    end

    self.scrollContainer = vgui_Create('MagieReroll_ScrollContainer', self)
    self.scrollContainer:SetPos(scale(191), scale(190))
    self.scrollContainer:SetSize(self:GetWide() - scale(382), scale(126))
    
    self.rerollButton = vgui_Create('MagieReroll_Button', self)
    ...
    timer_Create("MagieReroll_MoneyUpdate_"..LocalPlayer():EntIndex(), 1, 0, function()
        if IsValid(self) then
            self.money = LocalPlayer():getDarkRPVar('money') or 0
        else
            timer_Remove("MagieReroll_MoneyUpdate_"..self:EntIndex())
        end
    end)
end

function PANEL:SetupNetworkHooks()
    self.uniqueID = 'MagieReroll_Frame_' .. os_time() .. '_' .. math_random(1000, 9999)
    ...
    net_Receive('MagieReroll_SendResult', HandleRollResult)
end
...
function PANEL:Paint(w, h)
    ...
    MagieReroll.UI.Text('Menu Reroll - Magie', 'mr.font1', scale(210), scale(111), colors.main, 0, 3)
...
function PANEL:OnRemove()
    if timer_Exists("MagieReroll_MoneyUpdate_"..LocalPlayer():EntIndex()) then
        timer_Remove("MagieReroll_MoneyUpdate_"..LocalPlayer():EntIndex())
    end
end
...
vgui_Register('MagieReroll_Frame', PANEL, 'EditablePanel')