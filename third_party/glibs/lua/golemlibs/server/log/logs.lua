	...
	if not tables[tableName].existingColumns[name] then
		...
		if GOLEM.SQL.usingSQLite then
			...
			if GOLEM.SQL.db then
				local q = GOLEM.SQL.db:query(string.format("ALTER TABLE %s ADD COLUMN %s %s", tableName, name, columnTypes[type]))
				q:start()
			end
		end
...
function gLogs(tableName, ...)
	...
	if GOLEM.SQL.usingSQLite then
		...
		if not GOLEM.SQL.db then return false end
		...
		local q = GOLEM.SQL.db:prepare(string.format("INSERT INTO %s (%s) VALUES (%s)",
			tableName,
			table.concat(columns, ", "),
