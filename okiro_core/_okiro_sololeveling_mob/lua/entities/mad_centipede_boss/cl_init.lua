...
local ModelPath = "models/mad_centipedeboss.mdl"
...
local function DrawHealthBar(ent)
    ...
    local boneId = ent.ClientModel:LookupBone("Bone_Centiped_001")
...
hook.Add("PostDrawOpaqueRenderables", "DrawSoloLevelingNPCHealthBarCentipedeBoss", function()
    for _, ent in ipairs(ents.FindByClass("mad_centipede_boss")) do
        if IsValid(ent) then
            DrawHealthBar(ent)
        end
    end
